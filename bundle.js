...
var App = function () {
    ...
    _createClass(App, [{
        ...
        value: function clear() {
            var $newMain = (0, _jquery2.default)(this.container.cloneNode(false));
        ...
        value: function loadPeopleRapi() {
            ...
            _jquery2.default.ajax({
                ...
                success: function success(data) {
                    _this.outputPeople(data.results);
                }
            });
        }
    }, {
        ...
        value: function getPersonId($current) {
            ...
            var checkClass = function checkClass(el) {
                return el.hasClass(_this2.personСlassName) ? el.attr('data-id') : false;
            };
            return checkClass($current) || checkClass($current.parent());
        }
    }, {
        key: 'fillPersonModal',
        value: function fillPersonModal($modal, person) {
        ...
        value: function openModal(people) {
            ...
            return function (event) {
                ...
                if (id) {
                    ...
                    _this3.fillPersonModal($modal, person);

                    (0, _jquery2.default)('#btn-save_local').off('click.saveLocally');

                    var callback = person.hasOwnProperty('noSave') && !person.noSave ? function () {
                        _this3.saveInLocal(person);
                    } : function () {
                        alert('Already saved');
                    };

                    (0, _jquery2.default)('#btn-save_local').on('click.saveLocally', callback);
                }
            };
        }
    }, {
        ...
        key: 'fillPersonCard',
        value: function fillPersonCard($person, personData, id) {
            var $personCard = $person.clone(true);

            $personCard.attr('data-id', id);

            $personCard.find('.' + this.personСlassName + '-pic').attr('src', personData.picture.medium);

            $personCard.find('.' + this.personСlassName + '-name').html(personData.name.first + ' ' + personData.name.last);

            return $personCard;
        }
    }, {
        key: 'outputPeople',
        value: function outputPeople(people) {
            this.clear();

            var $fragment = (0, _jquery2.default)(document.createDocumentFragment());
            var $person = this.createPersonElemet('div');

            $person.append(this.createPersonElemet('img', 'pic'));
            $person.append(this.createPersonElemet('span', 'name'));

            for (var i = 0; i < people.length; i++) {
                var $user = this.fillPersonCard($person, people[i], i);
...
var app = new _app2.default(20, 'main');
